...
import com.ssmhdssmhd.mxboxs.databinding.FragmentSettingBinding;
...
import com.ssmhdssmhd.mxboxs.ui.activity.ScanActivity;
...
import com.ssmhdssmhd.mxboxs.ui.dialog.ThemeDialog;
...
public class SettingFragment extends BaseFragment implements ConfigListener, SiteListener, LiveListener, ThemeDialog.Listener {
    ...
    private static final int REQ_SCAN_TG = 0x401;
    private static final int REQ_SCAN_X  = 0x402;
    ...
    private void onParseServer(View view) {
        ...
        TextInputLayout til = new TextInputLayout(requireActivity());
    ...
    private void onSocialTg(View view) {
        String[] items = new String[] {
            getString(R.string.setting_social_scan),
        ...
        new MaterialAlertDialogBuilder(requireActivity())
                ...
                .setItems(items, (d, w) -> {
                    switch (w) {
                        case 0: ScanActivity.startForResult(this, REQ_SCAN_TG, Setting.SOCIAL_PURPOSE_TG); break;
                        case 1: pasteTokenTo(SOCIAL_TARGET_TG); break;
                        case 2: showChannelListDialog(); break;
                        case 3: clearSocialToken(SOCIAL_TARGET_TG); break;
                    }
                })
                .setNegativeButton(R.string.dialog_negative, null)
                .show();
    ...
    private void onSocialX(View view) {
        ...
        new MaterialAlertDialogBuilder(requireActivity())
                ...
                .setItems(items, (d, w) -> {
                    switch (w) {
                        case 0: ScanActivity.startForResult(this, REQ_SCAN_X, Setting.SOCIAL_PURPOSE_X); break;
                        case 1: pasteTokenTo(SOCIAL_TARGET_X); break;
                        case 2: showXEndpointDialog(); break;
                        case 3: clearSocialToken(SOCIAL_TARGET_X); break;
                    }
                })
                .setNegativeButton(R.string.dialog_negative, null)
                .show();
    ...
    private void onSocialTest(View view) {
        ...
        mSocialExec.execute(() -> {
            ...
            if (!Setting.isTgConnected() && !Setting.isXConnected()) {
                sb.append("\n请先点 Telegram Bot / X 卡片扫码或粘贴 Token 后再试。");
            }
    ...
    private void pasteTokenTo(int target) {
        ...
        if (cd != null && cd.getItemCount() > 0 && cd.getItemAt(0) != null) {
            CharSequence cs = cd.getItemAt(0).coerceToText(requireActivity());
    ...
    public void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        String text = data.getStringExtra(ScanActivity.EXTRA_RESULT_ADDRESS);
        String purpose = data.getStringExtra(ScanActivity.EXTRA_RESULT_PURPOSE);
        if (TextUtils.isEmpty(text)) return;
        if (requestCode == REQ_SCAN_TG || Setting.SOCIAL_PURPOSE_TG.equals(purpose)) {
            if (text.startsWith("tg://") || text.startsWith("https://t.me/")) {
                Notify.show("请扫描 @BotFather 给出的 Bot Token 文本二维码，而不是 Bot 链接二维码");
                return;
            }
            Setting.putTgBotToken(text);
            Notify.show("TG Bot Token 已保存");
            refreshSocialStatus();
            return;
        }
        if (requestCode == REQ_SCAN_X || Setting.SOCIAL_PURPOSE_X.equals(purpose)) {
            Setting.putXBearerToken(text);
            Notify.show("X Bearer Token 已保存");
            refreshSocialStatus();
        }
    }
