...
[PlaybackActivity.onScrubStop](file:///workspace/app/src/main/java/com/ssmhdssmhd/mxboxs/ui/activity/PlaybackActivity.java#L343-L350) 旧代码松手只调 `play()` **从不 seek**：进度条拖动只做视觉移动，松手后 `updateTimeBar` 把它画回原播放位置 → 表现为"拖不动/跳回"；且先 `setScrubbing(false)` 再 seek 顺序错误导致进度条先闪回旧位置。

**修复**：拖拽松手先 `mController.seekTo(position)`（`COMMAND_SEEK_IN_CURRENT_MEDIA_ITEM` 命令保护，直播跳过），再关 scrubbing。

### 问题 2 · 缩略图（拖动预览）不显示画面

[FrameExtractor](file:///workspace/app/src/main/java/com/ssmhdssmhd/mxboxs/utils/FrameExtractor.java) 旧实现用 `MediaMetadataRetriever` 取帧，但 **MMR 不支持 m3u8(HLS)**（`setDataSource` 直接抛异常），而 `114.134.184.91:8080/api/jx/server` 解析出的正是 `/api/play?url=...m3u8` → 缩略图永远空白；并发取帧访问同一个 MMR 实例还会崩溃/返回 null。
