...
### v5.7.7 · 2026-08-22 · 内置解析线路改为从仓库根目录文件拉取，接口配置支持文件/自定义切换

- **来源切换**：高级设置→接口配置顶部新增「接口来源」，可手动选择「文件调用（GitHub）」或「自定义线路」。
- **云端线路文件**：从当前仓库 main 分支根目录拉取 `nzbfq.txt`（播放器线路）与 `nzbfqjson.txt`（JSON 接口），一行一个、`#` 为注释，可随时增删无需改包。
- **播放优先级**：先调 `nzbfq.txt` 里的播放器，**全部播放失败后**才调 `nzbfqjson.txt` 的 JSON 接口；文件模式播放时自动启用 AI 去广告（过滤 OP/ED 等非正片内容）。
- **国内访问优化**：GitHub 直连失败自动依次尝试镜像池（与 OTA 更新同一套多镜像兜底），可手动「刷新文件线路并生效」（10 分钟内存缓存）。

版本号：versionCode 634 → **635** / versionName 5.7.6 → **5.7.7**
